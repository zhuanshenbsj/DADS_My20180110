...
public class PkgDecMHSport8OneLeaf extends PkgDecMHSportsComponent {
	...
	public PackageData getPackage(IoBuffer buffer) {
		...
		//new protocol add 21 bytes put DeviceId
		String firmware_version = DeviceIDResolver.getFirmwareVersion(buffer.array(), 18);
		String prefix = DeviceIDResolver.getDeviceIDPrefix(buffer.array(), 45);
		String deviceID = DeviceIDResolver.getDeviceIDFromBytes(buffer.array(), 50);
		...
		Logger.writeLog("in method handleNo8DataForOneWay deviceID:" + deviceID + " patientID:" + patientID);
